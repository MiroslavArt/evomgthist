...
class Common
{
	...
	public static function syncHoockDeals()
	{
		...
		$arOptionsSync = $connection->query($sSql)->fetchAll();

		// print_r($arrayIDDeals);
		$extclass = new Chstate($lsDealStatus, $arrayIDDeals, self::$propByOffer, $arOptionsSync);
		$ret = $extclass->updatefuncByLoop();
